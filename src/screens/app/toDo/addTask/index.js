...
const AddTask = (props) => {
    ...
    const onPressDoneBtn = async () => {
        if(validations()){
            ...
            await saveDataWithoutDocId('Tasks' , obj).then((data)=>{
                saveData('Tasks' ,data._documentPath._parts[1],{
    ...
    return (
        <ImageBackground style={styles.container} source={appImages.backGround}>
            <View style={{ backgroundColor: 'rgba(0,0,0,0.9)', height: '100%', width: '100%' }}>
                ...
                <MyTextInput placeholder={'Task name'} onchange={(txt)=>{setTastName(txt)}} err={taskNameError}/>
                <Text style={[styles.toptxt, { color: colors.txtInputborder, marginTop: responsiveHeight(4) }]}>{'Task Details'}</Text>
                <MyTextInput multiline={true} onchange={(txt)=>{setTaskDetals(txt)}} err={taskDetailErr}/>
                <View style={styles.buttonView}>
                    <AppButtonLarge Title={'Add Task'} onPress={onPressDoneBtn} activity={loading}/>
                </View>
                </ScrollView>
            </View>
        </ImageBackground>
