...
import { getData, removeItemfromArray, saveData, updateArray } from '../../../../Backend/utility'
...
const EditBank = (props) => {
    ...
    const prsDelBtn = async () => {
        let uid = await getCurrentUserId()
        await removeItemfromArray('Users' , uid , 'BankDetails' , index).then(()=>{
    ...
    const AddBank = async() => {
        ...
        //        })
        //    }) 
        await updateArray('Users' , uid , 'BankDetails' , obj , index).then(()=>{
            props.navigation.navigate('Bank')
    ...
    return (
        <ImageBackground style={styles.container} source={appImages.backGround}>
            <View style={{ backgroundColor: 'rgba(0,0,0,0.9)', height: '100%', width: '100%' }}>
                ...
                <MyTextInput placeholder={'Write name'} onchange={(text) => { handleOnChangeNameText(text) }} err={bankNameError} value={bankName}/>
                <Text style={[styles.toptxt, { color: colors.txtInputborder, marginTop: responsiveHeight(2) }]}>{'Account Number'}</Text>
                <MyTextInput multiline={true} placeholder={'Write account number'} onchange={(text) => { handleOnChangeAccountNumberText(text) }} err={accountNumberError} type={'numeric'} value={accountNumber}/>
                <Text style={[styles.toptxt, { color: colors.txtInputborder, marginTop: responsiveHeight(2) }]}>{'Branch Code'}</Text>
                <MyTextInput multiline={true} placeholder={'Write Branch Code'} onchange={(text) => { handleOnChangeBranchCodeText(text) }} err={branchCodeError} value={branchCode}/>
                <Text style={[styles.toptxt, { color: colors.txtInputborder, marginTop: responsiveHeight(2) }]}>{'Zip Code'}</Text>
                <MyTextInput multiline={true} placeholder={'Write zip code'} onchange={(text) => { handleOnChangeZipCodeText(text) }} err={zipCodeError} type={'numeric'} value={zipCode}/>
                ...
                <TouchableOpacity style={styles.delbtn} onPress={prsDelBtn}>
                    <Icon name={'delete'} type={'antdesign'} color={'red'} size={fontSize.h1}/>
                </TouchableOpacity>
                <View style={styles.buttonView}>
                    <AppButtonLarge Title={'Save'} onPress={AddBank} activity={loading}/>
                </View>
                </ScrollView>
            </View>
        </ImageBackground>
